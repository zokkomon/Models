...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan  8 21:27:24 2024
...
class TrainVQGAN:
    ...
    def train(self, args):
        ...
        for epoch in range(args.epochs):
            with tqdm(range(len(training_data))) as p_bar:
                for i, imgs in zip(p_bar, training_data):
                    ...
                    pcep_loss = self.perceptual_loss(imgs, decoded_imgs)
                    rec_loss = torch.abs(imgs - decoded_imgs)
                    pcep_rec_loss = args.perceptual_loss_factor*pcep_loss + args.rec_loss_factor*rec_loss
                    g_loss = -torch.mean(pcep_rec_loss)
                    
                    lmbda = self.vqgan.calculate_lambda(pcep_rec_loss, g_loss)
                    vq_loss = pcep_rec_loss + q_loss + disc_factor * lmbda * g_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--codebook-vectors', type=int, default=1024, help='Number of codebook vectors (default: 256)')
    ...
    args.dataset_path = r"/home/zok/joker/vinyAsa/layoutlm_data/images_"
