...
for epoch in tqdm(range(config.EPOCHS), position=0, leave=True):
    ...
    for idx, (img, tag) in enumerate(tqdm(dataloader, position=0, leave=True)):
        input_ids = tokenizer.encode(tag, return_tensors='pt')
        attention_mask = torch.ones(input_ids.size())
        img = img.float().to(config.device)
        label = input_ids.type(torch.uint8).to(config.device)
        y_pred = model(img)
